...
describe('App State Sync', () => {
	describe('missing key errors are marked with isMissingKey', () => {
		...
		it('missing key errors are not irrecoverable on first attempt', () => {
	...
	describe('isAppStateSyncIrrecoverable', () => {
		it('should not be irrecoverable for status 400', () => {
			expect(isAppStateSyncIrrecoverable(new Boom('test', { statusCode: 400 }), 1)).toBe(false)
		})

		it('should not be irrecoverable for status 404', () => {
		...
		it('should handle null and undefined error gracefully', () => {
